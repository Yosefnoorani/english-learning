        ...
        <div className="flex flex-col min-w-0 flex-1">
          ...
          <div className="flex items-center gap-2 mt-1">
            <div
              ...
              title={`Tier ${currentTier} of 10 · ${tierCorrectStreak} of ${tierTarget} correct`}
            >
              <div
            ...
            <span
              className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap hidden sm:inline"
              title={`Tier ${currentTier} of 10`}
            >
              {tierCorrectStreak}/{tierTarget}
            </span>
          </div>
        </div>

        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          <div className="flex items-center gap-1 bg-orange-50 dark:bg-orange-950/40 rounded-lg sm:rounded-xl px-2 sm:px-3 py-1 sm:py-1.5 min-h-[40px]">
            ...
            {streakFreezes > 0 && (
              <span className="text-[10px] text-orange-400 hidden sm:inline" title={`${streakFreezes} streak freeze${streakFreezes > 1 ? 's' : ''}`}>
